# ¡Hola! Soy José D. Monge 👋

### Técnico de Sistemas Junior | Administrador de Sistemas en formación (ASIR)

Tras casi 20 años coordinando equipos y servicios en el sector social, he decidido volcar mi experiencia en la gestión de personas y resolución de conflictos al mundo de la tecnología. Actualmente me especializo en la administración de infraestructuras críticas y soporte técnico.

---

## 🛠️ Stack Técnico

### 💻 Sistemas y Virtualización
* **Sistemas Operativos:** Windows Server, Ubuntu Server, Linux (Debian, Ubuntu, etc).
* **Virtualización:** VMware, VirtualBox.
* **Directorio Activo:** Gestión de usuarios y políticas en Active Directory.

### ☁️ Cloud & Redes
* **Cloud:** Microsoft Azure (**Certificación AZ-900**).
* **Networking:** Configuración de redes TCP/IP, resolución de incidencias de red.

### 📜 Herramientas de Gestión
* **Monitorización:** Zabbix (Despliegue y gestión de alertas).
* **Seguridad:** Hardening básico (UFW, Fail2ban).
* **Scripting:** Automatización básica con Bash, PowerShell y Python.

---

## 🌟 Mi Diferencial: De la Gestión Social al Soporte IT
* **Atención al Usuario:** Empatía y comunicación efectiva en situaciones de estrés.
* **Troubleshooting:** Capacidad analítica para resolver incidencias complejas.
* **Gestión de Crisis:** Experiencia real liderando equipos en entornos críticos.

---

## 📜 Certificaciones y Formación

### 🐧 Linux & Open Source
* **LPIC-1: Linux Administrator** ![LPIC-1](https://img.shields.io/badge/LPI-LPIC--1-black?style=flat&logo=linux)
  > [Verificación en Credly](https://www.credly.com/badges/accfa59e-334e-402b-8228-fa64c329782c)

### ☁️ Cloud & Microsoft
* **Microsoft Certified: Azure Fundamentals (AZ-900)** ![Azure](https://img.shields.io/badge/Microsoft-AZ--900-blue?style=flat&logo=microsoft-azure)
  > [Verificación en Microsoft Learn](https://learn.microsoft.com/es-es/users/josdanielmongemartnez-7049/credentials/certification/azure-fundamentals?tab=credentials-tab)

### 🎓 Formación Académica
* **Técnico Superior en Administración de Sistemas Informáticos en Red (ASIR)** *En curso - Centro de Estudios [Tu Centro]*

---

## 📫 Conecta conmigo:
* 📍 **Madrid, España** (Disponibilidad inmediata).
* 💼 **Objetivo:** IT Support | Help Desk | Junior SysAdmin | Cloud Junior.

* 💼 **LinkedIn:** [linkedin.com/in/josé-daniel-monge-b32bb5135](https://www.linkedin.com/in/josé-daniel-monge-b32bb5135)
* 📧 **Email:** jdmonge10@gmail.com
* 📍 **Ubicación:** Madrid, España
